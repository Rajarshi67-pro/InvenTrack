...
exports.productsController = {
    async getAll(req, res, next) {
        try {
            if (dbDown())
                return ok(res, { data: MOCK_PRODUCTS, total: MOCK_PRODUCTS.length, page: 1, limit: 20, totalPages: 1, hasNext: false, hasPrev: false });
    ...
    async getById(req, res, next) {
        try {
            if (dbDown())
                return ok(res, MOCK_PRODUCTS.find(p => p.id === req.params.id) || MOCK_PRODUCTS[0]);
    ...
    async update(req, res, next) {
        try {
            if (dbDown())
                return ok(res, { id: req.params.id, ...req.body }, "Product updated");
    ...
    async getLowStock(req, res, next) {
        try {
            if (dbDown())
                return ok(res, { data: MOCK_PRODUCTS.filter(p => p.quantity <= p.minStockLevel), total: 3 });
